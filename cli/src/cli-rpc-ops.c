			...
			int count, void *myframe)
{
	gf_cli_rsp           rsp   = {0,};
        ...
        gf_log ("cli", GF_LOG_INFO, "Received resp to ganesha");
	...
	if (rsp.op_ret) {
        if (strcmp (rsp.op_errstr, ""))
                        cli_err ("global option: failed: %s", rsp.op_errstr);
                else
                        cli_err ("global option: failed");
        }

	else

	cli_out("global option : success ");
...
int32_t
gf_cli_ganesha (call_frame_t *frame, xlator_t *this,
			void *data)
{
        gf_cli_req              req =  { {0,} } ;
	int                     ret = 0;
